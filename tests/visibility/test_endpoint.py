...
ENDPOINT_CONFIG = ['home', 'about', 'baz.foo', 'qux.foo']
...
@pytest.mark.options(STRONGHOLD_PUBLIC_ENDPOINTS=ENDPOINT_CONFIG)
@pytest.mark.parametrize('view_name', PUBLIC_ENDPOINT_VIEWS)
def test_configured_endpoints_are_visible(stronghold, client, view_name):
...
def test_configured_endpoints_are_not_visible(stronghold, client, view_name):
    resp = client.get(url_for(view_name))
...
def test_configured_endpoints_are_visible_for_logged_user(
